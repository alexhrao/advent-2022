use std::{ops::{Add}, collections::HashSet, fmt::Display};
...
impl Blizzard {
    pub fn step(&mut self, dimensions: &(usize, usize)) {
        ...
        if test.0 == 0 || test.0 == (dimensions.0 - 1) || test.1 == 0 || test.1 == (dimensions.1 - 1) {
            ...
            line
                ...
                .filter_map(move |(x, c)| {
                    ...
                    if let Ok(direction) = res {
                        Some(Blizzard { direction, location: (x, y) })
                    } else {
                        None
                    }
                })
        }).collect();
    let start = (map.lines().next().unwrap().chars().position(|c| c == '.').unwrap(), 0usize);
    let dest = (map.lines().last().unwrap().chars().position(|c| c == '.').unwrap(), dims.1 - 1);
    let seed = BoardState { minute: 0, party: start, blizzards };

    let a = bfs(&seed, |board| {
        ...
        }
        {
            ...
            if test.0 != (dims.0 - 1) && (test.1 != 0 || test == start) && !blizzards.contains(&test) {
                out.push(test);
            }
        }
        if party.1 != 0 {
        ...
        out
            .into_iter()
            .map(move |party| BoardState { minute: starter.minute, party, blizzards: starter.blizzards.clone(), })
    }, |x| x.party == dest);
    let a = a.unwrap();
    let len = a.len();
    for a in a {
        //print_board(&a, &dims, &start, &dest);
    }
    // 186 is too LOW
    println!("{}", len);
}
